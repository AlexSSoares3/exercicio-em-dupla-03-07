...
if (numero % 2 === 0)
   console.log(`O número é PAR você venceu!`)
else {
...
switch (dia) {
   case "Segunda": console.log(`Pastel R$ 5`);
   break;
...
let pagamentoCinema = (estudante === true) ? "Meia: R$ 15" : "Inteira"
console.log(pagamentoCinema)
...
function dividirPizza(valor, pessoas) {
  ...
  if(pessoas <= 0) return ("Precisa de pelo menos uma pessoa 1 pessoa!")
...
let km = 12
let calc = Number(km * 2) + 5  

if (total = calc * 0.9) {
   console.log(`Valor da Corrida: ${total}`)
}
...
if (fimDeSemana || brasilJoga && grana >= 50) {
